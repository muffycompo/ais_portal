        ...
        @if(! is_null($students) && is_array($students) )
          @foreach($students as $student)
            <tr>
                ...
                <td>{{ $student['total'] }}</td>
                <td>
                    {{ HTML::link('results/new_assessment/' . $student['student_id'] .'/' . $subject_id. '/' .$class_id,'Assessment Scores') }}
                    @if($student['total'] > 0)
                        @if($student['publish_status'])
                         | {{ HTML::link('results/unpublish_result/' . $student['student_id'] .'/' . $subject_id. '/' .$class_id,'UnPublish Result') }}
                        @else
                        | {{ HTML::link('results/publish_result/' . $student['student_id'] .'/' . $subject_id. '/' .$class_id,'Publish Result') }}
                        @endif
                    @endif
                </td>
            </tr>
